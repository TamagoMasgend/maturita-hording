...
- **Objektové databáze**: Inženýrský pokus implementovat do perzistence na pevné disky přímo softwarové objekty z RAM paměti bez mapování zpět do tabulek. Tyto modely trpěly složitostí správy a komerčně masivně prohrály s relačním modelem.
...
- *NoSQL platformy*: **MongoDB** (světová špička dokumentových databází využívaná v JavaScript MERN stacku), **Redis** (in-memory klíč-hodnota databáze pro cachování weba) a Neo4j (grafový systém).
