...
- **BGP (Border Gateway Protocol)**: Jediný zástupce vnějších směrovacích protokolů (Exterior Gateway Protocol). Je to "protokol, který pohání samotný Internet". Internet nedělí na lokální sítě, ale na rozsáhlý **Autonomní systémy (AS)** přidělené korporacím a ISP (mají unikátní AS Number). Směrovací tabulky BGP na páteřních kořenových routerech dnes obsahují statisíce záznamů. Metrika zde není primárním určovatelem trasy; směrování závisí na sérii atributů definujících komerční, geografické a bezpečnostní politiky dohodnuté mezi telekomunikačními operátory.