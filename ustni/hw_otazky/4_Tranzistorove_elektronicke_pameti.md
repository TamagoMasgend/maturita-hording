...
- **CL (CAS Latency):** Doba (v taktech), než paměť začne odesílat vyžádaná data. Je to nejdůležitější parametr (čím menší číslo, tím rychlejší odezva).
- **tRCD (RAS to CAS Delay):** Zpoždění při hledání přesného místa. Paměť je jako tabulka – toto je čas mezi vybráním řádku a přechodem na správný sloupec.
- **tRP (RAS Precharge):** Čas potřebný pro "úklid". Doba nutná k bezpečnému uzavření aktuálního řádku, než můžeme otevřít jiný.
- **tRAS (Active to Precharge Delay):** Minimální čas, po který musí řádek zůstat otevřený, aby se data stihla správně zpracovat a nedošlo k chybě.
